...
import VerifiedAnimation from "../components/auth/VerifiedAnimation";
...
export default function Login() {
  ...
  const [registrationDialog, setRegistrationDialog] = useState<"options" | "online" | null>(null);
    ...
    <section className="flex w-full max-w-md flex-col items-center gap-6" aria-labelledby="login-title">
      ...
      <form className="flex w-full flex-col gap-4" onSubmit={submit} noValidate aria-busy={pending}>
        <Input label="E-mail" type="email" autoComplete="username" maxLength={254} value={email}
          ...
          error={errors.email} disabled={pending} aria-required="true" inputSize="lg"
        ...
        <Input label="Senha" type="password" autoComplete="current-password" maxLength={128} value={password}
          ...
          error={errors.password} disabled={pending} aria-required="true" inputSize="lg"
          containerClassName="gap-4" labelClassName="text-base! font-normal! leading-6 text-black! dark:text-black!"
          inputClassName="h-10 bg-white py-2! text-black select-text dark:border-slate-200! dark:bg-white! dark:text-black! dark:disabled:border-slate-200! dark:disabled:bg-slate-50! dark:disabled:text-slate-400! dark:aria-invalid:border-red-500!" />
        ...
        <Button type="submit" size="lg" className="w-full shadow-none" disabled={pending} isLoading={pending}>
      ...
      <AuthDialog
        ...
        actions={
          <Button variant="secondary" size="lg" className="w-full shadow-none dark:bg-slate-100! dark:text-slate-700! dark:hover:bg-slate-200! dark:active:bg-slate-300!" onClick={() => setRegistrationDialog(null)}>
            Voltar
          </Button>
        }
      >
        <div className="mt-6 flex flex-col gap-6">
          <section className="flex flex-col gap-3 rounded-xl border border-primary-100 bg-primary-50 p-5">
            <p className="font-medium text-slate-800">Você faz parte de alguma empresa? Realize o Cadastro Online</p>
            <Button size="lg" className="w-full shadow-none" onClick={() => setRegistrationDialog("online")}>
              Cadastrar online
            </Button>
          </section>
          <section className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-5">
            <p className="font-medium text-slate-800">Caso não, realize o Cadastro Offline</p>
            <Button variant="secondary" size="lg" className="w-full shadow-none dark:bg-slate-100! dark:text-slate-700! dark:hover:bg-slate-200! dark:active:bg-slate-300!" onClick={() => { setRegistrationDialog(null); navigate("/cadastro", { viewTransition: true }); }}>
              Cadastrar offline
            </Button>
          </section>
        </div>
      ...
      <AuthDialog
        open={registrationDialog === "online"}
        onClose={() => setRegistrationDialog(null)}
        title="Cadastro on-line"
        actions={
          <Button variant="secondary" size="lg" className="w-full shadow-none dark:bg-slate-100! dark:text-slate-700! dark:hover:bg-slate-200! dark:active:bg-slate-300!" onClick={() => setRegistrationDialog("options")}>
            Voltar
          </Button>
        }
      >
      ...
      <AuthDialog
        ...
        illustration={<VerifiedAnimation active={registrationComplete} />}
        actions={<Button size="lg" className="w-full shadow-none" onClick={closeSuccess}>Fazer login</Button>}
      >
        <p className="text-center">Faça login na sua conta utilizando as credenciais e comece a utilizar o app agora.</p>
      </AuthDialog>
    </section>
  );
}
